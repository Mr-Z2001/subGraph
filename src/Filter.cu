...
void getEncoding(int nodesCount,
  ...
  for (int i = start; i < end; ++i) {
    l = edges_labels[i];
    printf("l: %d\n", l);
    encoding[node_id * encodingSize + l] += 1;
  }
...
//  cudaDeviceSynchronize();
  ...
  for (int i = node_id * encodingSize; i < (node_id + 1) * encodingSize; ++i)
